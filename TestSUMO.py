# -*- coding: utf-8 -*-
"""
Created on Wed Aug 21 13:14:02 2024
...
for i in range(1):
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if len(vehicle_ids)>2:
            #turn lane change model back on
            for vehicle_id in vehicle_ids:
                 traci.vehicle.setLaneChangeMode(vehicle_id,0b1010101010)
        
        #set up to know when change occurs
        if SUMO_change==False:
            if traci.vehicle.getLaneIndex("veh1")==1:
                env.change=True
                SUMO_change=True
...
#save data
#np.save('FVtoLVSUMO5.npy', data1)
#np.save('LVtoRVSUMO5.npy', data2)
#np.save('timesSUMO5.npy',times)
